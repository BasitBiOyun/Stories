...
- [x] Moses A2 — architecture-only migration complete. EN: 18 → 7 files. AR: 18 → 7 files. Active Quick Challenges, Knowledge Check, Vocabulary Challenge, Language Review and Final Challenge are consolidated in each language's `exercises.ts`; active Self-Study preview exports and their base guide are consolidated into `selfStudyGuide.ts`; Language Focus Parts 2–6 are consolidated into `languageFocus2.ts` and Parts 7–11 into `languageFocus3.ts`; base `languageFocus.ts`, story `pages.ts` and Teacher Guide blobs remained unchanged; `index.ts` imports only canonical files and preserves the existing BookData assembly behavior. Architecture migration SHA before this tracker handoff: `4011cc9ab0959fbd040e6709db706db8e5772cb1`.
- [x] Mecca/Bilal A2 — architecture-only migration complete. EN: 19 → 7 files. AR: 19 → 7 files. Active Quick Challenges, polished Knowledge Check, polished Vocabulary Challenge, Language Review and polished Final Challenge are consolidated in each language's `exercises.ts`; Language Focus Parts 2–6 are consolidated into `languageFocus2.ts` and Parts 7–12 into `languageFocus3.ts`; base `languageFocus.ts`, story `pages.ts`, Teacher Guide and Self-Study Guide blobs remained unchanged; `index.ts` imports only canonical files and preserves the existing BookData assembly behavior. Architecture migration SHA before this tracker handoff: `9facb05814a3aff6dfc7a57ad7a939ead77dc87d`. A2 architecture queue complete.
...
- [x] Abraham B1 — architecture-only migration complete. EN: 20 → 7 files. AR: 20 → 7 files. Active exercise-system and Language Review exports are consolidated in each language's `exercises.ts`; Language Focus Chapters 3–8 are consolidated in `languageFocus2.ts` and Chapters 9–13 in `languageFocus3.ts`; English `selfstudyGuide.ts` was canonicalized to `selfStudyGuide.ts` with the same blob; Teacher Guide, Self-Study content and consolidated learning-content blobs were not rewritten in this closeout. `index.ts` now imports only canonical files and assembles the existing B1 exercise/Language Focus wiring without the previous index-level media fallback helper. Architecture closeout SHA before this tracker handoff: `3880385a103cb249e573c0dfd866420f0a682657`.
- [x] Moses B1 — architecture-only migration complete. EN: 18 → 7 files. AR: 18 → 7 files. Active Quick Challenges, Knowledge Check, Vocabulary Challenge, Language Review, Final Challenge and existing polished assessment exports are consolidated into each language's `exercises.ts`; Language Focus Chapters 3–8 are consolidated into `languageFocus2.ts` and Chapters 9–13 into `languageFocus3.ts`; base `languageFocus.ts`, `pages.ts`, Teacher Guide and Self-Study Guide blobs remained unchanged; `index.ts` imports only canonical files and preserves the existing BookData assembly behavior. Architecture migration SHA before this tracker handoff: `bda7a0f56c0b41b6729b527e582ed35e15b9a671`.
- [ ] Mecca/Bilal B1 — next B1/B2 architecture target.
