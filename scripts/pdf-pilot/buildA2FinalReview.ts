...
<header class="review-header">
  <div><div class="review-eyebrow">${escapeHtml(eyebrow)}</div><h1>${escapeHtml(title)}${continuation ? `<span>${escapeHtml(continuation)}</span>` : ''}</h1><p>${escapeHtml(subtitle)}</p></div>
...
const renderSkillsPage = (page: PageData, language: Language): string => {
  ...
  const sequenceItems = sequenceOrder.map((index) => sequencing.sequencingItems?.[index]).filter(Boolean);
  const pairs = matching.matchingPairs ?? [];
  const shuffledRights = matchingOrder.map((index) => pairs[index]?.right).filter((value): value is string => Boolean(value));
  const bankLetters = isArabic ? ['أ','ب','ج','د'] : ['A','B','C','D'];

  return `<article class="review-page${isArabic ? ' review-rtl' : ''}">
    ${renderPageHeader(page.title, page.content, isArabic ? 'المراجعة النهائية' : 'FINAL REVIEW', isArabic ? ' • ١' : ' • 1')}
    <section class="review-block sequencing-block">
      ...
      <h2>${escapeHtml(sequencing.title || '')}</h2><p class="instruction">${escapeHtml(sequencing.instructions || '')}</p>
      <div class="sequence-list">${sequenceItems.map((item, index) => `<div class="sequence-row"><span class="sequence-id">${isArabic ? ['أ','ب','ج','د','هـ'][index] : String.fromCharCode(65 + index)}</span><span>${escapeHtml(item?.text || '')}</span><i></i></div>`).join('')}</div>
      <div class="sequence-answer-line"><strong>${escapeHtml(isArabic ? 'الترتيب الصحيح:' : 'My order:')}</strong><span></span><span></span><span></span><span></span><span></span></div>
    </section>
    <section class="review-block behavior-block">
      ...
      <h2>${escapeHtml(matching.title || '')}</h2><p class="instruction">${escapeHtml(matching.instructions || '')}</p>
      <div class="behavior-bank">${shuffledRights.map((item, index) => `<span><b>${escapeHtml(bankLetters[index] ?? String(index + 1))}</b> ${escapeHtml(item)}</span>`).join('')}</div>
      <div class="behavior-targets">${pairs.map((pair) => `<div class="behavior-target"><strong>${escapeHtml(pair.left)}</strong><i></i></div>`).join('')}</div>
    </section>
  </article>`;
};
...
const renderReflectionPage = (page: PageData, language: Language): string => {
  ...
  const modeNames: Record<string, string> = isArabic ? { Individual: 'فردي', Pair: 'ثنائي', Class: 'صفي' } : { Individual: 'Individual', Pair: 'Pair', Class: 'Class' };
  return `<article class="review-page${isArabic ? ' review-rtl' : ''}">
    ${renderPageHeader(page.title, isArabic ? 'فكر، اكتب، ثم شارك أفكارك.' : 'Think, write, then share your ideas.', isArabic ? 'التفكير والتأمل' : 'THINK & REFLECT', isArabic ? ' • ٢' : ' • 2')}
    <section class="reflection-intro"><h2>${escapeHtml(reflection.title || '')}</h2><p>${escapeHtml(reflection.instructions || '')}</p></section>
    <section class="reflection-list">${prompts.map((prompt, index) => `<div class="reflection-item"><div class="reflection-meta"><span>${isArabic ? ['١','٢','٣'][index] : index + 1}</span><strong>${escapeHtml(modeNames[prompt.mode] || prompt.mode)}</strong></div><h3>${escapeHtml(prompt.question)}</h3><div class="writing-lines"><i></i><i></i><i></i></div></div>`).join('')}</section>
  </article>`;
};
...
const renderQuizQuestion = (question: NonNullable<Exercise['quizQuestions']>[number], number: number, language: Language): string => {
  ...
  return `<div class="quiz-question">
    ...
    <div class="quiz-copy"><h3>${escapeHtml(question.question)}</h3><div class="quiz-options">${options.map((option, index) => `<span><b>${escapeHtml(letters[index] ?? String(index + 1))}</b><em>${escapeHtml(option.text)}</em></span>`).join('')}</div></div>
  </div>`;
};
...
const renderQuizPages = (page: PageData, language: Language): string => {
  ...
  const chunks = [questions.slice(0, 4), questions.slice(4, 8)].filter((chunk) => chunk.length);
  return chunks.map((chunk, pageIndex) => `<article class="review-page${isArabic ? ' review-rtl' : ''}">
    ${renderPageHeader(quiz.title || page.title, pageIndex === 0 ? (quiz.instructions || '') : (isArabic ? 'أكمل الأسئلة الأربعة الأخيرة.' : 'Complete the final four questions.'), isArabic ? 'تحدي المراجعة' : 'REVIEW CHALLENGE')}
    <section class="quiz-list quiz-list-four">${chunk.map((question, index) => renderQuizQuestion(question, pageIndex === 0 ? index + 1 : index + 5, language)).join('')}</section>
  </article>`).join('');
};
...
const renderDocument = (pages: PageData[], language: Language): string => {
  ...
  const isArabic = language === 'ar';
  return `<!doctype html><html lang="${language}" dir="${isArabic ? 'rtl' : 'ltr'}"><head><meta charset="utf-8"/><title>Adam A2 Final Review</title><link rel="stylesheet" href="a2-final-review.css"/></head><body class="lang-${language}">${renderSkillsPage(page, language)}${renderReflectionPage(page, language)}${renderQuizPages(page, language)}</body></html>`;
};
