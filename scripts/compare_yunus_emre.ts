...
function comparePages(level: string, enPages: any[], arPages: any[]) {
  log(`\n=== Comparing Yunus Emre ${level} Pages ===`);
  ...
  log(`Finished Yunus Emre ${level}. Total mismatches/warnings: ${totalMismatches}`);
}

log("Starting comparison for Yunus Emre...");
comparePages("A2", yunusA2Pages, yunusEmreA2PagesAr);
comparePages("B1", yunusB1Pages, yunusEmreB1PagesAr);
comparePages("B2", yunusB2Pages, yunusEmreB2PagesAr);

fs.writeFileSync('scripts/yunus_emre_comparison_report.txt', report);
log("\nSaved report to scripts/yunus_emre_comparison_report.txt");
