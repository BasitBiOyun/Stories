...
const auditBook = async (definition: (typeof bookRegistry)[number]): Promise<AuditResult> => {
  const label = `${definition.storyId}:${definition.level}`;
  const [pair, assets] = await Promise.all([
    definition.load(),
  ...
  for (const id of storyIds) {
    ...
    if (!en || ar?.type !== 'story') {
      contradictions.push(`${label} chapter ${id}: EN/AR story page pairing missing.`);
      continue;
    }
    ...
    if (en.image !== ar.image) contradictions.push(`${label} chapter ${id}: EN/AR images differ.`);
    if (en.audioUrl && ar.audioUrl && en.audioUrl === ar.audioUrl) contradictions.push(`${label} chapter ${id}: EN/AR narration URL is identical.`);
  }

  const total = storyIds.length;
  return {
    summary: `${label}: images ${images}/${total}${missingImages.length ? ` missing[${missingImages.join(',')}]` : ''} | EN audio ${englishAudio}/${total}${missingEnglishAudio.length ? ` missing[${missingEnglishAudio.join(',')}]` : ''} | AR audio ${arabicAudio}/${total}${missingArabicAudio.length ? ` missing[${missingArabicAudio.join(',')}]` : ''}`,
    contradictions,
  };
};

const main = async () => {
  console.log('[Storage media audit] scanning configured Firebase folders concurrently...');
  const results = await Promise.all(bookRegistry.map(auditBook));
  results.forEach(result => console.log(`[Storage media audit] ${result.summary}`));
  const contradictions = results.flatMap(result => result.contradictions);
  contradictions.forEach(message => console.error(`[Storage media audit] ${message}`));
  if (contradictions.length) throw new Error(`[Storage media audit] found ${contradictions.length} chapter-level media contradictions.`);
