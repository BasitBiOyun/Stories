...
import { meccaB1LearningBlueprint } from '../../src/data/mecca/b1/learningBlueprint';
...
const cases: BlueprintCase[] = [
  {
    label: 'Adam B1', storyId: 'adam', blueprint: adamB1LearningBlueprint, config: adamB1BlueprintConfig,
    rawEn: adamB1Pages, rawAr: adamB1PagesAr, bookEn: adamB1BookDataEn, bookAr: adamB1BookDataAr,
    targets: adamB1HighlightTargets, tapIds: ['adam-b1-c10-quick', 'adam-b1-c5-quick'],
  },
  {
    label: 'Moses B1', storyId: 'moses', blueprint: mosesB1LearningBlueprint, config: mosesB1BlueprintConfig,
    rawEn: mosesB1Pages, rawAr: mosesB1PagesAr, bookEn: mosesB1BookDataEn, bookAr: mosesB1BookDataAr,
    targets: mosesB1HighlightTargets, tapIds: ['moses-b1-c12-quick', 'moses-b1-c3-quick'],
  },
  {
    label: 'Abraham B1', storyId: 'abraham', blueprint: abrahamB1LearningBlueprint, config: abrahamB1BlueprintConfig,
    rawEn: abrahamB1Pages, rawAr: abrahamB1PagesAr, bookEn: abrahamB1BookDataEn, bookAr: abrahamB1BookDataAr,
    targets: abrahamB1HighlightTargets, tapIds: ['abraham-b1-c10-quick', 'abraham-b1-c5-quick'],
  },
  {
    label: 'Mecca B1', storyId: 'mecca', blueprint: meccaB1LearningBlueprint, config: meccaB1BlueprintConfig,
...
const validateLanguage = (
  ...
  if (config.vocabularyPageId) {
    ...
    assert.equal(vocabularyPage.vocabularyPairs?.length, 10, `${label}: Vocabulary in Context must contain 10 pairs.`);
  }
  ...
  config.glossaryPageIds.forEach((id, index) => {
    ...
    assert.deepEqual(glossary.vocabulary ?? [], expected, `${label}: glossary ${index + 1} is not derived from final Word Notes.`);
  });
};

for (const current of cases) {
  const { label, storyId, blueprint, config, rawEn, rawAr, bookEn, bookAr, targets, tapIds } = current;
  ...
  assert.equal(allItems.length, config.storyIds.length + 26, `${label}: authored item count mismatch.`);
  ...
  const pointIds = allItems.map(item => item.learningPointId);
  assert.equal(new Set(pointIds).size, pointIds.length, `${label}: an assessed learning point is repeated.`);
  ...
  assert.equal(reviewTypes.filter(type => type === 'multiple-choice').length, 4, `${label}: Review must contain 4 MC items.`);
  assert.equal(reviewTypes.filter(type => type === 'true-false').length, 4, `${label}: Review must contain 4 True/False items.`);
  assert.equal(new Set(reviewTypes).size, 2, `${label}: Review may only use MC and True/False.`);

  const finalTypes = new Set(stageItems('final').map(item => item.exercise.en.type));
  assert.deepEqual([...finalTypes].sort(), ['fill-blanks', 'matching', 'multiple-choice', 'true-false'], `${label}: Final must preserve four scored interaction types.`);
  ...
  for (const chapter of blueprint.chapters) {
    ...
    assert.equal(chapter.evidencePoints.length, chapter.assessmentItems.length, `${label} Chapter ${chapter.chapterId}: evidence/item count mismatch.`);
    ...
    assert.equal(chapter.vocabularyTargets.length, canonicalTargets.length, `${label} Chapter ${chapter.chapterId}: Word Note count mismatch.`);
    chapter.vocabularyTargets.forEach((target, index) => {
      ...
      assert.equal(normalizeHighlightText(target.en.word, 'en'), normalizeHighlightText(canonical.en.word, 'en'), `${label} Chapter ${chapter.chapterId}: English Word Note changed.`);
      assert.equal(normalizeHighlightText(target.ar.word, 'ar'), normalizeHighlightText(canonical.ar.word, 'ar'), `${label} Chapter ${chapter.chapterId}: Arabic Word Note changed.`);
      assert.equal(target.en.definition, canonical.en.definition, `${label} Chapter ${chapter.chapterId}: English definition changed.`);
      assert.equal(target.ar.definition, canonical.ar.definition, `${label} Chapter ${chapter.chapterId}: Arabic definition changed.`);
    });
  }
