...
const A2_VOCABULARY_COUNT = getLearningLevelPolicy('A2').vocabularyCount;
...
const pickTargetVocabulary = (entries: A2VocabularyEntry[]): A2VocabularyEntry[] => {
  const unique = uniqueEntries(entries);
  if (unique.length <= A2_VOCABULARY_COUNT) return unique;
  ...
  for (const entry of unique) {
    if (selected.length >= A2_VOCABULARY_COUNT) break;
    const key = keyOf(entry.word);
    if (seen.has(key)) continue;
    seen.add(key);
    selected.push(entry);
  }
  return selected.slice(0, A2_VOCABULARY_COUNT);
};
...
export const applyA2VocabularyGold = ({
  ...
  const challengeWords = pickTargetVocabulary(storyVocabulary);

  return withWordNotes.map((page) => page.id === vocabularyPageId
    ? {
        ...page,
        content: language === 'ar'
          ? 'تدرّب على عشر كلمات مستهدفة من القصة من خلال المعنى والسياق والاسترجاع.'
          : 'Practise ten target story words through meaning, context, and recall.',
        vocabularyPairs: challengeWords.map((entry) => ({
          word: entry.word,
          meaning: entry.definition,
          context: entry.storyExample ?? entry.example,
