...
export type B1HighlightLanguage = 'en' | 'ar';
type HighlightText = NonNullable<PageData['vocabulary']>[number];

export interface B1ExplicitHighlightPair {
  id?: string;
  en: HighlightText;
  ar: HighlightText;
}

export interface B1CanonicalHighlightTarget {
  id: string;
  chapterId: number;
  en: HighlightText;
  ar: HighlightText;
...
export interface B1HighlightStandardConfig {
  storyKey: string;
  storyIds: number[];
  glossaryPageIds: [number, number];
  vocabularyPageId?: number;
  /**
   * Chapters listed here use only these reviewed bilingual pairs. This is the
   * escape hatch for legacy B1 books whose old EN/AR Word Notes were selected
   * independently and therefore cannot safely be paired by array position.
   */
  explicitTargets?: Record<number, readonly B1ExplicitHighlightPair[]>;
}

export interface B1HighlightStandardResult {
...
const fail = (storyKey: string, message: string): never => {
  throw new Error(`[${storyKey} B1 highlight contract] ${message}`);
};
...
const syncDerivedVocabularyPages = (
  pages: PageData[],
  config: B1HighlightStandardConfig,
  language: B1HighlightLanguage,
): PageData[] => {
  ...
  return pages.map((page) => {
    ...
    if (glossaryIndex !== -1) {
      return {
        ...
        content: language === 'ar'
          ? glossaryIndex === 0
            ? 'جَمِيعُ الْكَلِمَاتِ وَالْعِبَارَاتِ الْمُظَلَّلَةِ فِي النِّصْفِ الْأَوَّلِ مِنَ الْقِصَّةِ لِلْمُرَاجَعَةِ.'
            : 'جَمِيعُ الْكَلِمَاتِ وَالْعِبَارَاتِ الْمُظَلَّلَةِ فِي النِّصْفِ الثَّانِي مِنَ الْقِصَّةِ لِلْمُرَاجَعَةِ.'
          : glossaryIndex === 0
            ? 'All highlighted B1 words and phrases from the first half of the story for review.'
            : 'All highlighted B1 words and phrases from the second half of the story for review.',
...
/**
 * B1 uses the same reader contract as A2:
 * - English owns the learning-target set.
 * - Arabic supplies the exact story surface for the same concept.
 * - one runtime mechanism only (`vocabulary`); legacy `animatedWords` disappear.
 * - a target hidden by the reader because it was already seen is removed from
 *   both languages, keeping visible bilingual counts identical.
 * - Master Glossary and Vocabulary-in-Context pages derive from final targets.
 *
 * Legacy chapters whose EN/AR Word Notes were already translated in parallel may
 * use positional pairing. Any chapter with independently selected metadata must
 * provide `explicitTargets` instead of relying on position.
 */
export const applyB1HighlightStandard = (
  englishPages: PageData[],
  arabicPages: PageData[],
  config: B1HighlightStandardConfig,
): B1HighlightStandardResult => {
  const seenEnglishKeys = new Set<string>();
  const seenArabicKeys = new Set<string>();
  const targets: Record<number, readonly B1CanonicalHighlightTarget[]> = {};
  ...
  for (const chapterId of config.storyIds) {
    ...
    const chapterTargets: B1CanonicalHighlightTarget[] = [];
    const chapterEnglishWords: string[] = [];

    const candidatePairs: B1ExplicitHighlightPair[] = explicit
      ? explicit.map((pair) => ({
          ...pair,
          en: { ...pair.en },
          ar: { ...pair.ar },
    ...
    for (const pair of candidatePairs) {
      ...
      // StoryPage suppresses an exact target after an earlier chapter. If Arabic
      // would be suppressed, suppress its paired English target too.
      ...
      const target: B1CanonicalHighlightTarget = {
    ...
    if (!chapterTargets.length) {
      fail(config.storyKey, `Chapter ${chapterId} has no canonical bilingual B1 highlight targets.`);
    }
    if (chapterTargets.length > 5) {
      fail(config.storyKey, `Chapter ${chapterId} has ${chapterTargets.length} targets; B1 allows at most 5.`);
    }

    targets[chapterId] = chapterTargets;
  }

  const applyLanguage = (pages: PageData[], language: B1HighlightLanguage): PageData[] => {
  ...
  const result: B1HighlightStandardResult = {
    targets,
    englishPages: applyLanguage(englishPages, 'en'),
    arabicPages: applyLanguage(arabicPages, 'ar'),
  };
  validateB1HighlightStandard(result.englishPages, result.arabicPages, result.targets, config);
  return result;
};

export const validateB1HighlightStandard = (
  englishPages: PageData[],
  arabicPages: PageData[],
  targets: Record<number, readonly B1CanonicalHighlightTarget[]>,
  config: B1HighlightStandardConfig,
  ...
  for (const chapterId of config.storyIds) {
    ...
    if (enPage.animatedWords?.length || arPage.animatedWords?.length) {
      record(`Chapter ${chapterId} still contains legacy animatedWords.`);
    }
    ...
    expected.forEach((target, index) => {
      ...
      if (!enEntry.definition?.trim() || !arEntry.definition?.trim()) {
        record(`Chapter ${chapterId} target ${target.id} has an empty learner definition.`);
      }
      if (!highlightPhraseOccurs(enPage.content ?? '', enEntry.word, 'en')) {
        record(`English Chapter ${chapterId} target “${enEntry.word}” is not in story prose.`);
      }
      if (!highlightPhraseOccurs(arPage.content ?? '', arEntry.word, 'ar')) {
        record(`Arabic Chapter ${chapterId} target “${arEntry.word}” is not in story prose.`);
      }

      const arKey = normalizeHighlightText(arEntry.word, 'ar');
      const previous = seenArabicRuntimeTargets.get(arKey);
      if (arKey && previous && previous !== target.id) {
        record(`Arabic target “${arEntry.word}” is reused by ${previous} and ${target.id}; the reader would hide one.`);
      }
  ...
  config.glossaryPageIds.forEach((pageId, index) => {
    ...
    if (actualEn.length !== expectedEn.length || actualAr.length !== expectedAr.length) {
      record(`Glossary page ${pageId} is not synchronized with final story highlights.`);
    }
    expectedEn.forEach((entry, entryIndex) => {
      const actual = actualEn[entryIndex];
      if (!actual
        || normalizeHighlightText(actual.word, 'en') !== normalizeHighlightText(entry.word, 'en')
        || actual.definition !== entry.definition) {
    ...
    expectedAr.forEach((entry, entryIndex) => {
      ...
      if (!actual
        || normalizeHighlightText(actual.word, 'ar') !== normalizeHighlightText(entry.word, 'ar')
        || actual.definition !== entry.definition) {
  ...
  if (config.vocabularyPageId) {
    ...
    if (actualEn.length !== expectedEn.length || actualAr.length !== expectedAr.length) {
      record(`Vocabulary page ${config.vocabularyPageId} is not synchronized with final highlights.`);
    }
  }
