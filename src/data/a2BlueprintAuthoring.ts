...
const feedback = (explanation: LocalizedText) => ({
  en: {
    correct: 'Correct. This answer is supported by the chapter.',
    incorrect: `Return to the chapter and check this detail: ${explanation.en}`,
  },
  ar: {
    correct: 'صحيح. هذه الإجابة يدعمها نص الفصل.',
...
export const fill = (
  ...
  return {
    en: {
      ...
      instructions: 'Complete the sentence with a word or phrase from the chapter.',
      question: question.en,
    ...
    ar: {
      type: 'fill-blanks',
      title: 'أكمل الجملة',
      instructions: 'أكمل الجملة بكلمة أو عبارة من الفصل.',
      question: question.ar,
...
export const guideBundle = (input: A2GuideInput): { teacherGuide: BlueprintGuidePair; selfStudyGuide: BlueprintGuidePair } => ({
  teacherGuide: {
    en: {
      ...
      lessonPlan: `Preview the title and image. Read once for the main event. Reread to answer: ${input.primaryQuestion.en} Check Word Notes in context, complete the Quick Challenge, then discuss: ${input.secondQuestion.en} End with one short factual sentence supported by the chapter.`,
    ...
    ar: {
      ...
      lessonPlan: `عاين العنوان والصورة. اقرأ مرة لفهم الحدث الرئيس. أعد القراءة للإجابة عن: ${input.primaryQuestion.ar} راجع ملاحظات المفردات في السياق، ثم نفّذ التحدي السريع وناقش: ${input.secondQuestion.ar} اختم بجملة واقعية قصيرة يدعمها الفصل.`,
  ...
  selfStudyGuide: {
    en: {
      ...
      grammarFocus: input.grammar.en,
      pronunciationFocus: input.pronunciation.en,
      lessonPlan: `Read the chapter once without stopping. Read it again and answer: ${input.primaryQuestion.en} Guess highlighted words from context before opening Word Notes. Complete the Quick Challenge. If unsure, reread the exact sentence. Finish by answering: ${input.secondQuestion.en}`,
      discussionPoints: [input.primaryQuestion.en, input.secondQuestion.en],
      differentiation: {
        fastFinishers: input.fast.en,
        strugglingLearners: input.support.en,
